...
自定义扩展组件：droplist
改造jeasyui的combogrid插件的keyHandler和onShowPanel等事件，实现海量数据的异步处理方法，支持搜索

依赖库：scrollview、combogrid
...
*****************************************************************/
(function($){
	$.fn.droplist = function (options, param) {//定义扩展组件
		...
		return this.each(function () {
			...
			options.onShowPanel = function(){
				var style = options.style,
					...
					url = options.droplisturl,
					grid = jq.combogrid('grid'),
...
*/			
			options.keyHandler = {
				...
				enter: function(){
					...
					if (selected && co.combo('getValue') != '') {//已经选中有值的，关闭下拉框，跳转到下一个可编辑控件
						...
						//if(ops.template || ops.droplisturl){
			...
			options.onValidate = function(v){//验证成功后调用事件
				if(v){
					var co = $(this);
			...
			var myopts = $.extend(true, options, opts);
			$.fn.combogrid.call(jq, myopts);
		});
	}
	$.parser.plugins.push("droplist");//注册扩展组件
