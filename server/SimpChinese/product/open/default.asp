...
(function(){
	...
	if(find.length > 0){
		var sql ="select top 1 [bd].[code] as code,[bd].[title] as title,[bd].[id] as [id] ,[bd].[code] as [productcode],[bd].[title] as [productname] ,[bd].[nvarchar1] as [unit] ,[bd].[nvarchar2] as [Spec] ,[bd].[nvarchar3] as [nvarchar3] ,[bd].[currency7] as [price] ,[bd].[currency6] as [detailprice],[bd].[currency7] as [amount] ,[bd].[taxrate] as [taxrate] ,[bd].[isdeleted] as [isdeleted] ,[bd].[nvarchar1] as [nvarchar1] ,[bd].[searchcode1] as [searchcode1] ,[bd].[currency10] as [currency10] ,[bd].[nvarchar4] as [nvarchar4] ,[bd].[vendercode] as [vendercode] ,[bd].[vipcash] as [vipcash] ,1 as [quantity],bd.LengthSerial from biproduct [bd], resProductStock res where res.productid=bd.id and res.ProductSerial='"+find+"' and [bd].[isdeleted]=0 and  [bd].[accountid]=1 and [bd].[isdeleted]=0",
			rs = ebx.dbx.open(sql);
